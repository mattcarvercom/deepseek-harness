...
| `@deepseek-ai/dsh-tool-attachment` | `attachment_paths` | `ctx.tools`, `ctx.attachments (registration and execution)` | `tool/call`, `tool/result` | - | Returns read-only host paths for the images and files attached to the most recent human message; the tool is not registered without a mounted attachment store. |
...
Return absolute host paths for the images and files attached to the user's most recent message that carries attachments. Call this when the user attached an image and a path-based tool needs it (image editing, reading, shell commands). The returned paths are read-only stored copies; pass one directly to a tool that reads files. Attached non-image files already carry their saved path in the conversation, so this tool is mainly how you get a path for an attached image.
...
Source: [`packages/attachment/tool-attachment/src/index.ts`](../packages/attachment/tool-attachment/src/index.ts)

Returns read-only host paths for the images and files attached to the most recent human message; the tool is not registered without a mounted attachment store.
