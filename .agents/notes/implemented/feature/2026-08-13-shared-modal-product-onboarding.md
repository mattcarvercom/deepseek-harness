...
**One existing client Cordis plugin owns both shipped steps.** `ui-settings-models` registered `welcome-notice` at order `-100` and `deepseek-official` at order `0` in `settings.onboarding`; the [testing-notice removal](../simplification/2026-09-05-remove-gui-testing-notice.md) later deleted the `welcome-notice` entry, leaving `deepseek-official` at order `0` as the plugin's only registrant. The shell continues to mount only the first incomplete entry, so the dialogs cannot stack. No additional client package or plugin row is introduced.
...
**The welcome notice reused the existing durable field.** Its exact copy and version lived in `onboarding-copy.ts`; loopback clients compared and wrote `ui-onboarding.welcomeNoticeVersion` through the existing settings API, and only Continue acknowledged the current version. The [testing-notice removal](../simplification/2026-09-05-remove-gui-testing-notice.md) deleted this step, its component, its store, and its copy owner entirely; the Host `ui-onboarding` namespace registration remains only so a settings document still carrying an old `welcomeNoticeVersion` stays valid.
...
A fresh loopback profile saw the specified internal-testing notice, then an inline DeepSeek key dialog only when no provider was usable; the [testing-notice removal](../simplification/2026-09-05-remove-gui-testing-notice.md) later dropped the first dialog, so a fresh profile now goes straight to the inline DeepSeek key dialog when no provider is usable. Secrets remain write-only in `.credentials.yaml`, and already-ready or unsupported deployments render no onboarding chrome while readiness loads. The Models package still owns product-onboarding presentation as well as provider configuration; its README and browser coverage make that responsibility explicit. This decision had restored a concise testing-stage notice after the historical [full-viewport beta notice removal](../../archived/simplification/2026-08-13-remove-first-run-beta-notice.md) without restoring that notice's telemetry copy or takeover layout — a restoration the later removal above undid in turn.
