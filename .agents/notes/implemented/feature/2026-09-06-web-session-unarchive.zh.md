...
[会话归档](../../archived/feature/2026-07-31-session-archive-global-set.md)会把会话从所有显示面隐藏，但被隐藏的会话无法被触及：没有任何视图列出它们，行菜单的归档行也没有反向操作。被归档的会话在暗中不断累积——用户既看不到自己隐藏了什么，更无法把一个会话请回来。归档 note 已把恢复面记为后续工作：一个 UI 面加一个反向 RPC。本 note 就是那个面。
