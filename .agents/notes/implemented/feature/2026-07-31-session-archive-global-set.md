...
The restore this note deferred shipped as [Session unarchive and the archived-sessions view](2026-09-06-web-session-unarchive.md): one UI surface (the view-options archived filter) plus one inverse RPC (`unarchiveSession`), on the retained slots and the full-snapshot echo exactly as the design left room for. The `workspace.list` response shape change is a pre-release direct edit (no compatibility layer). The workspace-management e2e pins the full chain (archive → row disappears → still hidden after reload, log still present); domain tests pin idempotence, unknown-id rejection, restart recovery, and the pre-field media default upgrade.
