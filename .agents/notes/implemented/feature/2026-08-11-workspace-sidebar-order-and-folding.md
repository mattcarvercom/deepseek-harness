...
**Keep independent Manual and Last updated orders.** Switching modes would replace the visible list with stale positions from the other order, even though choosing Manual only means that later activity stops moving rows. That tradeoff was revisited in [Last updated as a strict recency sort](2026-09-06-last-updated-strict-recency-sort.md), which renders Last updated without persisting an order of its own.
...
- Workspace order is durable and shared through the Host, while grouping, open state, per-account Session view order, and query state remain browser-local presentation preferences. Ungrouped and the flat list support the same drag rules, but their orders are browser-local because neither has one Workspace account.
- Last updated renders a strict recency sort at render time and leaves the persisted order untouched; Manual restores the stored arrangement — [Last updated as a strict recency sort](2026-09-06-last-updated-strict-recency-sort.md).
