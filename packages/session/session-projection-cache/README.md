...
### Removing a session's row

`remove(id)` permanently deletes one session's stored checkpoint row: it drops any pending write-behind state for the session, awaits every row replacement already enqueued (throttled, mandatory, or cold-read write-back) so the table delete lands after them, and then deletes the row. Unknown ids are a no-op. A row whose log was destroyed elsewhere is inert — no cold read can serve it — and `remove` is the in-band surface that clears it; for a still-live session the row reappears only through the ordinary write path, so callers dispose the owning agent first.
...
- **No bulk eviction or retention surface** — `remove(id)` deletes one session's row on demand, but bulk pruning of stored checkpoints (age, size, or count-based retention) is out-of-band maintenance.
