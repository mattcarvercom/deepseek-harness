...
| `startedAt` | 该轮 `turn/start` 事件的时间（epoch ms）——分页窗口不含该边界时，为运行中轮次的经过时钟提供持久锚点 |
...
该单元是对已提交会话事件的纯折叠。锚定每个条目的是 `turn/start` 而非提示词 `user/message`，因为它的 seq 就是跳转的载入目标：agent loop（智能体循环）先记 `turn/start` 再记该轮的提示词与步骤，窗口向后分页越过该 seq 即包含整轮。条目同时记录边界事件的时间，使分页窗口不含该边界的客户端也能把运行中轮次的经过时钟锚定到这个全量折叠已携带的事实上。提示词由首条人类 `user/message` 填充，且仅当最新条目仍为空时——同一轮内后续的人类消息（steering（中途引导））保留首个预览。回复无法同样填充（`turn/end` 不带文本），所以每条带文本的 `assistant/message` 覆写状态里的草稿，`turn/end` 提交幸存者——最新的文本，与已加载导航栏 `findLast` 的语义一致。
