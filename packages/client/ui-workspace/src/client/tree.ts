...
export interface SessionNode {
  ...
  /** Registry-archived session: dimmed and offering unarchive while the archived filter shows it. */
  archived: boolean
  updatedAt: number
}
...
export interface TreeView {
  ...
  /** Show registry-archived Sessions in their retained positions instead of hiding them. */
 ...
 * sessions are hidden unless the archived filter shows them, while their
...
function sessionVisible(
  ...
  return session.origin !== 'subagent'
    && (showArchived || !archived.has(session.id))
...
function groupByWorkspace(
  ...
  for (const workspace of workspaces) {
    ...
    for (const id of workspace.sessionIds) {
      ...
      accounted.add(id)
      if (!sessionVisible(summary, current, archived, showArchived)) continue
  ...
  const stray = list.ids
    ...
    .filter((s): s is SessionSummary =>
      s !== undefined && !accounted.has(s.id) && sessionVisible(s, current, archived, showArchived))
  if (stray.length > 0) {
    groups.push(buildGroup(
      UNGROUPED_KEY,
      undefined,
...
export function deriveGroups(
  ...
  for (const g of groupByWorkspace(list, workspaces, archived, view.ungroupedOrder, view.showArchived === true)) {
    ...
    groups.push({
      ...
      sessions: expanded
        ? g.sessions.map(session => sessionNode(session, descendants, statuses, archived))
        : [],
    })
  }
  return groups
...
export function visibleSessionIds(
  ...
  return list.ids.filter((id) => {
    ...
    return s !== undefined && sessionVisible(s, current, archived, showArchived)
  })
}
...
export function deriveFlat(
  ...
  const archived = new Set(archivedSessionIds)
  return sessionIds
    .map(id => sessionNode(list.byId[id] as SessionSummary, descendants, statuses, archived))
}
...
export function deriveSearchResults(
  ...
  for (const id of list.ids) {
    ...
    if (summary === undefined || summary.blank || !sessionVisible(summary, current, archived, false)) continue
    if (
  ...
  for (const item of content.items) {
    ...
    if (summary !== undefined && !summary.blank && sessionVisible(summary, current, archived, false)) include(summary)
  }
