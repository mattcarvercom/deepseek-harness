...
describe('workspace browser rows', () => {
  it('omits only an empty leading status slot in the hierarchy-free flat list', () => {
    ...
    const view = render(<SessionNodeItem node={idle} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} flat t={t} />)
    ...
    view.rerender(<SessionNodeItem node={{ ...idle, running: true }} currentId={undefined} now={0}
      onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} flat t={t} />)
  ...
  it('renders and opens a selected running Session row', () => {
    ...
    render(
      <SessionNodeItem node={node} currentId={node.id} now={0} onOpen={onOpen}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} t={t} />,
    )
  ...
  it('reveals a clipped session title by scrolling it while the row is hovered', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 0, completed: false, hasActiveSchedule: false, archived: false, updatedAt: 0,
    }
    render(
  ...
  it('keeps the active-Schedule marker between the title and time in grouped and flat rows', () => {
    ...
    const node: SessionNode = {
      ...
      runningSubagentCount: 0, completed: false, hasActiveSchedule: true, archived: false, updatedAt: 0,
    }
    const view = render(
    ...
    const renderRow = (over: Partial<SessionNode>) => render(
      <SessionNodeItem
        node={{
          ...
          runningSubagentCount: 0, completed: false, hasActiveSchedule: false, archived: false, updatedAt: 0, ...over,
        }}
  ...
  it('shows descendant activity without describing an idle parent as running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 2, completed: false, hasActiveSchedule: false, archived: false, updatedAt: 0,
      }
  ...
  it('keeps descendant activity secondary while the parent is running', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 1, completed: false, hasActiveSchedule: false, archived: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} t={t} />)
      const row = screen.getByRole('treeitem')
  ...
  it('keeps child activity as a secondary status while user attention is primary', () => {
    const node: SessionNode = {
      ...
      running: false, runningSubagentCount: 1, completed: false, hasActiveSchedule: false, archived: false, updatedAt: 0,
    }
    render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
      onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} t={t} />)
    const row = screen.getByRole('treeitem')
  ...
  it('session row menu opens without opening the session and dispatches rename, fork, archive, and delete', () => {
    ...
    const view = render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onArchive={onArchive} onUnarchive={onUnarchive} onDelete={onDelete} t={t} />)
    ...
    // The archived row swaps in the inverse action and dispatches it.
    view.rerender(<SessionNodeItem node={{ ...node, archived: true }} currentId={undefined} now={0} onOpen={onOpen}
      onRename={onRename} onFork={onFork} onArchive={onArchive} onUnarchive={onUnarchive} onDelete={onDelete} t={t} />)
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
    ...
    // Delete opens the confirmation (no commit here) with the current display title.
    fireEvent.click(screen.getByRole('button', { name: '会话“One”的操作' }))
  ...
  it.each([
    ...
    try {
      const node: SessionNode = {
        ...
        pendingInteraction, running: true, runningSubagentCount: 0, completed: false,
        hasActiveSchedule: false, archived: false, updatedAt: 0,
      }
      ...
      view.rerender(<SessionNodeItem node={{ ...node, running: false }} currentId={undefined} now={0}
        onOpen={vi.fn()} onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} t={t} />)
  ...
  it('completed hover card shows the Completed status line', () => {
    ...
    try {
      const node: SessionNode = {
        ...
        runningSubagentCount: 0, completed: true, hasActiveSchedule: false, archived: false, updatedAt: 0,
      }
      render(<SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} t={t} />)
      fireEvent.pointerEnter(screen.getByRole('treeitem').parentElement as HTMLElement)
  ...
  it('draggable row wires start/end and gates hover/drop on an active same-group drag', () => {
    ...
    const { rerender } = render(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} drag={inactive} t={t} />,
    )
    const row = screen.getByRole('treeitem')
    stubRect(row)
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} drag={active} t={t} />,
    )
    ...
    rerender(
      <SessionNodeItem node={node} currentId={undefined} now={0} onOpen={vi.fn()}
        onRename={vi.fn()} onFork={vi.fn()} onArchive={vi.fn()} onUnarchive={vi.fn()} onDelete={vi.fn()} drag={after} t={t} />,
    )
