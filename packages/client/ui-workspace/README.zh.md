...
Session 行内的 Rename 操作打开一个以该行显示标题预填的对话框；确认未修改的标题是有意允许的——这正是把当前自动标题钉住、不再被重新生成覆盖的手势。Rename 使用临时 `workspaceOperation` reference，fork 标题设置则由 Session Controller 使用临时 `controllerOperation` reference；两者都等待该 reference 的首次历史打开。Archive 不经确认对话框直接提交，归档集合回声落地后，该行从所有分组视图中消失；在**显示已归档会话**开启时，被提回的行改以暗淡样式渲染在其保留位置，其菜单提供反向的 Unarchive 操作，该操作不经对话框提交，回声落地后恢复普通行样式。Fork 在源会话最后一个已完成轮次处 fork，在客户端递增继承的持久化标题后再打开子会话。Session 删除会打开一个确认框，说明已存储日志及其全部内容将被销毁且操作不可撤销；确认后，Host 销毁日志且删除回声落地时，该行从所有视图中消失。Workspace 行内的 Delete 操作会打开确认框，说明保留边界；成功后该分组被移除，其 Session 则留在 Ungrouped 下。
...
- **Session 删除是唯一不可逆的操作** —— 已归档会话由「已归档会话」设置页（[ui-settings-unarchive-sessions](../ui-settings-unarchive-sessions/README.zh.md)）负责恢复；删除 Session 会销毁其存储日志且无法撤销，而删除 Workspace 注册只解除分组，其 Session 日志保持不变。
