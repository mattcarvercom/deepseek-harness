...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.chat.step-actions',
    kind: 'list',
    scope: 'session',
    summary: 'Ordered actions for one settled assistant step that is not its Turn\'s closing message — the working steps whose text the transcript shows while the agent runs.',
    doc: 'Ordered actions for one settled assistant step that is not its Turn\'s\nclosing message — the working steps whose text the transcript shows\nwhile the agent runs. Each entry receives the step\'s durable message\nid; a fresh `id` adds an action and reusing one replaces that entry.',
    registerOptions: [
      {
        name: 'id',
        requirement: 'required',
    ...
    occupants: [],
    replaceRisk: 'none',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.chat.step-actions\', () => ctx.slots.register(\n      { name: \'conversation.chat.step-actions\', id: \'my-entry\', order: 100, label: \'My entry\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/client/ui-chat/src/client/contract/slots.ts:346',
  },
  {
    key: 'conversation.chat.stream-actions',
    kind: 'single',
    scope: 'session',
    summary: 'Streaming highlight bridge for one generating assistant step, addressed by its stable node key: a feature marks the text being read while the step has no durable message id yet.',
    doc: 'Streaming highlight bridge for one generating assistant step, addressed\nby its stable node key: a feature marks the text being read while the\nstep has no durable message id yet.',
    registerOptions: [],
    ownerProps: [
