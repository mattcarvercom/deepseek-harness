...
export class FakeApiClient {
  ...
  onKillSubagent: (payload: unknown) => Promise<RemoteResult<{ accepted: true }>> =
  ...
  sessionRemotes(): RuntimeRemotes {
    return {
      ...
      session: {
        ...
        killSubagent: payload => this.record('session.killSubagent', payload, this.onKillSubagent(payload)),
