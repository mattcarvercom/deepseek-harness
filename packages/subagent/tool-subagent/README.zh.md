...
一次性后台模式会注册一个归父级所有的普通 Task，其 done 通道结算启动，并在 detail 中保留终止原因与可选提供方诊断。可继续后台模式调用 `ctx.subagents.startContinuable()`，该调用在 inbox 接受时结算：子 agent 自此拥有自己的轮次，因此该调用既不等待也不收集结果。一次性运行的活动载荷在启动确认后携带子代理的持久会话 id，使客户端能够提供终止操作；首条记录可能早于确认而不带该字段，进程外运行则从不携带。
