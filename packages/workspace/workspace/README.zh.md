...
当会话的日志已被销毁时，把它从全部记账中剥离：`removeSession(id)` 将它从每个项目记录中分离，从归档集合中清除，并丢弃其 header-index 条目，使之后的列表刷新无法在任何成员关系投影中复活该 id。它返回记录或归档集合是否发生变化；在何处都未记账的 id 是幂等空操作（index 条目仍会被丢弃）。
